...
Pass --lidar-only to skip all camera initialisation and record LiDAR data
only, while still producing the full session folder layout and metadata.
"""
...
class CameraEntry:
    ...
    directory: Path
    camera: Any  # TisCamera — imported lazily when not in lidar-only mode
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Capture software-synchronized LiDAR frames and stereo images. "
            "Use --lidar-only to record LiDAR data without any camera setup."
        )
    )
    parser.add_argument(
        "--frames",
        type=int,
        required=True,
        help="Number of frames to capture (LiDAR frames in --lidar-only mode, synchronized pairs otherwise)",
    )
    parser.add_argument(
        "--lidar-only",
        action="store_true",
        help="Skip stereo camera initialisation and capture LiDAR data only",
    )
    parser.add_argument(
        "--config",
        default=str(software_sync_root / "config" / "stereo_software_trigger.json"),
        help="Path to stereo camera JSON config (ignored when --lidar-only is set)",
    )
    parser.add_argument(
        "--output-root",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.35,
        help="Timeout per camera when waiting for a software-triggered frame (ignored with --lidar-only)",
    )
    parser.add_argument(
        "--lidar-code",
        default="",
    ...
    parser.add_argument(
        "--show-video",
        action="store_true",
        help="Show live ximagesink previews for each camera (ignored with --lidar-only)",
    )
...
def init_cameras(
    ...
    try:
        import cv2  # noqa: F401  # imported for side-effects in TisCamera
        from tis_camera import TisCamera
    except ImportError as error:
        raise RuntimeError(
            f"Stereo camera libraries not available ({error}). "
            "Run with --lidar-only to skip camera setup."
        ) from error
...
def main() -> int:
    ...
    if args.lidar_only:
        print("Mode: LiDAR only (stereo cameras disabled)")
    ...
    try:
            ...
            while True:
                ...
                if args.lidar_only:
                    frame_record: dict[str, Any] = {
                        ...
                        "lidar_event_time_ns": lidar_event_time_ns,
                        "lidar_frame_raw_index": lidar_raw_index,
                    }
                else:
                ...
                print(f"Captured frame {captured_frames}/{args.frames}")
        ...
        if len(records) != args.frames:
            raise RuntimeError(
                f"Requested {args.frames} frames but captured {len(records)}"
            )
        ...
        summary = {
            ...
            "config_path": str(args.config) if not args.lidar_only else None,
